#!/usr/bin/env node
/**
 * generate-dev-config.js — Generates staticwebapp.config.json (local dev)
 * from staticwebapp.config.swa.json (production / source of truth).
 *
...
// Skip in CI — the production config is managed by the deployment workflow.
if (process.env.CI) {
...
const SOURCE = path.join(ROOT, 'staticwebapp.config.swa.json');
const OUTPUT = path.join(ROOT, 'staticwebapp.config.json');
...
console.log('✅ Generated staticwebapp.config.json (local dev) from staticwebapp.config.swa.json');
